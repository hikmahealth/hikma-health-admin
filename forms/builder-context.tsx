...
const FormBuilderContext = React.createContext<null | [State, React.Dispatch<Action>]>(null);
...
type Action =
  ...
  | { type: 'set-form-state'; payload: { fields: HHField[] } }
  ...
  | { type: 'set-dropdown-options'; payload: { index: number; value: FieldOption[] } }
  | { type: 'set-field-key-value'; payload: { index: number; key: string; value: any } }
  | { type: 'add-units'; payload: { index: number; value: DoseUnit[] } }
...
export const FormBuilderContextProvider = function ({
  ...
  const ctx = React.useReducer(reducer, initialState ?? INITIAL_FORM_BUILDER_CONTEXT);
  return <FormBuilderContext.Provider value={ctx}>{children}</FormBuilderContext.Provider>;
};
